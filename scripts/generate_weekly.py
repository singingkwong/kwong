...
def fetch_weekly_html() -> str:
    ...
    prompt = (
        ...
        "3. 各地市场动态必须覆盖以下全部区域，不得以表格形式遗漏：北美、欧洲、中国、东南亚、南美（巴西/墨西哥）、印度、俄罗斯、澳洲、日韩。每个区域必须独立输出并且**每个区域恰好 5 条**新闻，各区域条数一致且不多不少都是 5 条；输出规范：【一个区域只输出一张卡片（card-title 只写区域名，如“北美”“欧洲”“中国”），卡内用 5 个段落/列表项列出该区域 5 条独立的一周市场事件/数据/政策，每条都必须有“关键数据|影响分析|趋势判断”命名字段并以“来源：机构名”结尾，且每条都带一个 source-link 原文链接】。严禁把一条事件单独拆成一张卡，严禁跳过任何区域，若该区域一周事件不足 5 条，可结合近两周数据或合理趋势预判补足到 5 条；\n"
