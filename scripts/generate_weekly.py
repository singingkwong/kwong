...
def fetch_weekly_html(attempt: int = 1, extra_instruction: str = "") -> str:
    ...
    prompt = (
        ...
        "7.6 图片要求：整个报告的 <img> 图片数量必须 ≥ 3 张（建议在'本周总览'、'各地市场动态'、'注塑机会专题'等板块配 1 张与内容匹配的真实配图），\n"
        "   - 所有配图必须是联网核验过、确实存在且能正常加载的真实图片 URL（来自权威媒体/机构官网），禁止使用无效占位图或随机图床地址；\n"
        "   - 每张 <img> 必须带 width/height 属性（比例 16:9 或 4:3）避免布局抖动，并带 alt 描述；\n"
        "   - 若某板块确实无合适配图，至少保证全报告 <img> ≥ 3 张，不得输出 0 张图;\n"
        "7.7 【重要】除'本周总览'外的其余模块（各地市场动态、政策动态、车企动态、调研报告/机构观点、注塑机会专题），每条新闻/动态的卡片正文必须写成'三要点'结构，让读者不点开原文也能拿到核心价值：\n"
        ...
        "    (c) 全篇至少要有 4+4+4+4+4=20 条带 source-link 链接的独立动态分散在上述 5 张区域卡内（允许更多，但不能少于 20）；全部板块（市场+政策+车企+调研+注塑）合计 <a class='source-link'> 原文链接数量必须 ≥ 26 条，且 <img> 配图数量必须 ≥ 3 张；\n"
        "    (d) 输出完成后逐条核对 (a)(b)(c)，若任一不满足，必须即时补充对应区域卡或列表项、补充缺少的原文链接或配图，直到 6 张卡、每卡 ≥4 条、共 ≥20 条、全篇原文链接 ≥26 条、配图 ≥3 张全部达标再结束生成。每一条事件的三要点（关键数据/事件、影响分析、趋势判断）与来源必须放在同一个 <li> 列表项内，严禁拆成多个 <li>。"
