...
6. **非市场板块非空态**：政策动态 / 车企动态 / 调研报告·机构观点 / 注塑机会专题 不得是"暂无/无新增/未更新"这类空态占位，否则判 FAIL 并纳入补缺重呼（`empty_sections()` 提取空态板块名传给下一次生成）。

> 校验器同时提供 `empty_sections(checks)` 返回空态板块名列表，供 `generate_weekly.py` 构造针对性补缺指令。
