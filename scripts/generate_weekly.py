...
def fetch_weekly_html() -> str:
    ...
    prompt = (
        ...
        "3. 各地市场动态必须覆盖以下全部区域，不得以表格形式遗漏：北美、欧洲、中国、东南亚、南美（巴西/墨西哥）、印度、俄罗斯、澳洲、日韩。每个区域必须独立输出 5 条新闻（一条区域对应一张卡片或一段，每条为该区域本周一条独立的真实市场事件/数据/政策），每条包含关键数据、趋势和注塑机会关联，各区域条数一致且不少于 5 条；\n"
