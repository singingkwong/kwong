...
def fetch_weekly_html() -> str:
    ...
    prompt = (
        ...
        "3. 各地市场动态：**只保留固定 7 个区域，顺序必须严格为：中国、北美、欧洲、南美、东南亚、印度、其他**。先把本周收集到的所有汽车行业相关新闻/事件/数据/政策，全部归入这 7 个区域之一；分类规则：【中国=中国内地市场；北美=美国+加拿大+墨西哥；欧洲=欧盟+英国+德法等欧洲国家；南美=巴西/阿根廷/墨西哥外其他国家等南美地区；东南亚=东盟国家（印尼/泰国/马来西亚/越南等）；印度=印度；其他=除上述明确区域外的其他地区（如俄罗斯/澳洲/日韩/中东等），不得把南美塞进北美，不得把日韩就近并入中国或欧洲，必须各归其类】。每个区域必须独立输出并且**每个区域至少 5 条**（news 总条数不设上限，各区域条数可不同，但每个展示的区域都不得少于 4 条，且 7 个区域一个都不能少：“其他”区域若真实事件不足，可说明本周无重大新增、仍应输出该区域占位卡）。输出规范：【一个区域只输出一张卡片（card-title 只写区域名，且只能是“中国”“北美”“欧洲”“南美”“东南亚”“印度”“其他”之一），卡内用至少 4 个 <li> 列表项列出该区域至少 4 条独立的一周市场事件/数据/政策，每条必须是一个完整的 <li>，且内部用“关键数据/事件：…影响分析：…趋势判断：…”三要点逐条解读（关键数据/事件给出具体数据/数字/时间/政策名；影响分析说明对汽车行业供应链/注塑机及材料/零部件的影响；趋势判断给出短期趋势或后续关注点），每条都带一个 <a class='source-link'> 原文链接并以“来源：机构名”结尾（三要点与来源写在同一个 <li> 内，严禁把一条事件的三个要拆成三个 <li>）】。严禁把一条事件的三要点拆成多个条目，严禁跳过或新增任何区域（最终只能有这 7 张区域卡，每张卡内至少 4 条且无上限），若某区域一周真实事件不足 4 条，可结合近两周数据或合理趋势预判补足到至少 4 条，但不得编造不存在的事件；“其他”区域可略少于 4 条。\n"
        ...
        "    (a) '<div class=\"card-title\">中国</div>'、'<div class=\"card-title\">北美</div>'、'<div class=\"card-title\">欧洲</div>'、'<div class=\"card-title\">南美</div>'、'<div class=\"card-title\">东南亚</div>'、'<div class=\"card-title\">印度</div>'、'<div class=\"card-title\">其他</div>' 七张区域卡必须**全部**出现在'各地市场动态'模块中，一张都不能省、不能合并、不能改名；\n"
        "    (b) 中国／北美／欧洲／南美／东南亚／印度 这 6 张卡，**每张卡内的 card-content 都必须包含至少 4 个 <li> 列表项**（每个 <li> 是一条独立市场动态，各含'关键数据/事件：…／影响分析：…／趋势判断：…'三要点 + '来源：机构名' + 一个 <a class='source-link'> 原文链接，且三者与来源必须在同一个 <li> 内）；'其他'区域卡可包含 1-4 条或说明本周无重大新增；\n"
        "    (c) 全篇至少要有 4+4+4+4+4+4=24 条带 source-link 链接的独立动态分散在上述 6 张区域卡内（允许更多，但不能少于 24）；\n"
        "    (d) 输出完成后逐条核对 (a)(b)(c)，若任一不满足，必须即时补充对应区域卡或列表项，直到 7 张卡、每卡 ≥4 条、共 ≥24 条全部达标再结束生成。每一条事件的三要点（关键数据/事件、影响分析、趋势判断）与来源必须放在同一个 <li> 列表项内，严禁拆成多个 <li>。"
    )
