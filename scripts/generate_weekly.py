...
def fetch_weekly_html() -> str:
    ...
    prompt = (
        ...
        "7.5 【重要】所有新闻/动态类内容都必须附直达原文链接：\n"
        "   - 适用范围：本周总览各热点、各地市场动态、政策动态、车企动态、调研报告/机构观点、注塑机会专题，凡涉及具体事件/数据/新闻的条目，每条都必须给出可跳转的原文链接；\n"
        "   - 输出格式：在该条目内部用 <a href='原文URL' class='source-link' target='_blank'>原文</a> 固定格式输出，链接必须是真实存在的原文网页 URL，禁止使用链接占位符或伪链接；\n"
        "   - 若你无法确认某条的真实原文 URL，请去掉该条或标注（来源待核），不要编造链接；\n"
